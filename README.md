...
**GitHub Actions** で毎日 0:00 JST に `batch:diff` が自動実行されます。リポジトリの **Settings > Secrets and variables > Actions** に `POKEMON_API_KEY`、`SUPABASE_URL`、`SUPABASE_SERVICE_ROLE_KEY` を設定してください。cron の例は **[docs/cron-example.sh](docs/cron-example.sh)** を参照してください。
...
1. **① sets（日本語セットのみ）**  
   `GET /api/v2/sets?language=japanese` で全日本語セットを取得。API の `id` を `sets.api_set_id` に、`tcgPlayerNumericId` を `sets.tcg_player_numeric_id` に保存（② で `setId` に使用）。毎日 or 週1 でOK。

2. **② cards（日本語カード）**  
   DB の `sets`（`language=japanese`）を参照し、各セットごとに  
   `GET /api/v2/cards?language=japanese&setId={sets.tcg_player_numeric_id}&fetchAllInSet=true` で取得。  
   `setId` は `tcgPlayerNumericId`。カード情報のみ取得（価格・履歴・PSA は ③ で別途取得）。  
...
5. **sealed（シールド商品）**  
   `GET /api/v2/sealed-products?language=japanese&search={term}&limit=100` を複数検索ワード（booster box, booster pack, elite trainer box 等）で実行し、全シールド商品を取得。DB の sets は不要。
