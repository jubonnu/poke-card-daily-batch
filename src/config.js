...
export const config = {
  api: {
    baseUrl: process.env.POKEMON_API_BASE_URL || 'https://www.pokemonpricetracker.com/api/v2',
    ...
    maxRetries: Math.max(0, parseInt(process.env.API_MAX_RETRIES ?? '3', 10)),
    /** リトライ初回待機ミリ秒（指数バックオフの基準）。未設定時 5000 */
    retryDelayMs: Math.max(1000, parseInt(process.env.API_RETRY_DELAY_MS || '5000', 10)),
  },
  supabase: {
  ...
  batch: {
    cardsPerRequest: parseInt(process.env.BATCH_CARDS_PER_REQUEST || '50', 10),
    ...
    sealedIncludeHistory: process.env.BATCH_SEALED_INCLUDE_HISTORY !== 'false',
    delayBetweenRequests: parseInt(process.env.BATCH_DELAY_MS || '2000', 10),
    /** USD→JPY 為替レート（バッチ保存時の円換算に使用。未設定時は 150） */
